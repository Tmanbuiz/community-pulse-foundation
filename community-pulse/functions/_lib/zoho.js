 ...
 */
/**
 * Queue a volunteer communication and return its row id.
 *
 * The volunteer-side counterpart to queueEnquiryComm. This INSERT used to be
 * hand-written in three separate files and had already drifted - one copy
 * omitted body_preview and created_by entirely - which is exactly the failure
 * a single writer prevents.
 *
 * `preview` is a short excerpt, omitted when there is nothing to preview
 * beyond the subject. `createdBy` is null for mail generated by the public
 * submission endpoint, where there is no admin actor to attribute it to.
 */
export function volunteerCommStatement(env, { volunteerId, type, to, subject, preview, createdBy, at }) {
  ...
  return env.DB
    ...
    .bind(volunteerId, type, to, subject || null, (preview || '').slice(0, 200) || null, createdBy || null, now, now);
}

/** Execute one queued row. Callers needing several should batch the statements. */
export async function queueVolunteerComm(env, options) {
  const row = await volunteerCommStatement(env, options).run();
